...
#include "CollectionOfDataAboutObject.h"
	...
	std::list<std::string> CollectionOfDataAboutObject::Collection_of_synchronization_data()
	{
			bool all = false; // false ="\\*.*";
							  //  true  = "\\*";
			// если пользователь ввел только диск 
			if (this->namaWhat.size() == 1)
			{
				this->namaWhat += ":";
				all = true;
			}
