...
namespace Example
{
	...
	void TcpServer::Bind(void)
	{
		...
		if (::bind(TcpBase::s, reinterpret_cast<sockaddr*>(&local_addr), sizeof(local_addr)) == SOCKET_ERROR)
			throw  Example::NetException("Error bind", ::WSAGetLastError());
	}

	void TcpServer::Listen(void)
	{
		if (::listen(TcpBase::s, this->iCountConnected) == SOCKET_ERROR)
			throw  Example::NetException(" Error listen", ::WSAGetLastError());
	}
	...
	SOCKET TcpServer::Accept(void)const
	{
		...
		if (s == INVALID_SOCKET)
			throw  Example::NetException(" Error accept", ::WSAGetLastError());
